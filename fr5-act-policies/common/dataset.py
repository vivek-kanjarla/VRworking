...
class FR5Dataset(Dataset):
    ...
    def __init__(self, root, chunk_size=100, use_image=True,
                 ...
                 aug_level="none", n_obs_steps=1, cameras=None):
        self.root        = Path(root)
        ...
        self.cameras     = cameras or ["wrist_cam"]      # camera(s) to load
        ...
        if self.use_image:
            ...
            if self.n_obs_steps == 1:
                ...
                for cam in self.cameras:
                    img = self._load_frame(cam, ep_idx, frame_in_ep, aug_seed)
                    if img is None:
                        print(f"[dataset] WARN: missing {cam} frame ep{ep_idx} idx{frame_in_ep}, using zeros")
                        img = torch.zeros(3, *self.image_size)
                    sample[f"observation.images.{cam}"] = img
            else:
                ...
                for step in range(self.n_obs_steps - 1, -1, -1):
                    ...
                    img = self._load_frame(self.cameras[0], ep_idx, frame_idx, frame_seed)
                    frames.append(img if img is not None else torch.zeros(
                        3, *self.image_size))
                # (n_obs_steps, C, H, W)  — single-camera only for the multi-step path
                sample[f"observation.images.{self.cameras[0]}"] = torch.stack(frames, dim=0)

        return sample

    def _load_frame(self, cam: str, ep_idx: int, frame_idx: int,
                    aug_seed: int | None = None) -> torch.Tensor | None:
        vkey = f"observation.images.{cam}"
        jpg = (self.root / "frames" / vkey /
               f"ep-{ep_idx:03d}" / f"{frame_idx:06d}.jpg")
        frame = cv2.imread(str(jpg)) if jpg.exists() else \
                self._read_video_frame(cam, ep_idx, frame_idx)
    ...
    def _read_video_frame(self, cam: str, ep_idx: int, frame_idx: int) -> np.ndarray | None:
        vid = self.root / "videos" / f"observation.images.{cam}" / "chunk-000" / f"file-{ep_idx:03d}.mp4"
        if not vid.exists():
            return None
