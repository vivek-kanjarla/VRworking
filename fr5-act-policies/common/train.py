...
def run_epoch(model, loader, optimizer, cfg, device, train=True):
    ...
    with ctx:
        for step, batch in enumerate(loader):
            ...
            imgs = {k.split("observation.images.")[1]: v.to(device)
                    for k, v in batch.items() if k.startswith("observation.images.")}
            imgs = imgs or None
            task  = batch.get("task")  # list[str] or None; used by language-conditioned policies

            # bf16 autocast on CUDA — faster + less VRAM (no GradScaler needed for bf16)
            with torch.autocast("cuda", dtype=torch.bfloat16, enabled=(device.type == "cuda")):
                loss, l1, kl = model(obs, acts, pad, imgs, task=task)
